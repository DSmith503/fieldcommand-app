...
export default function Schedule() {
  ...
  const [newForm, setNewForm] = useState({ user_id: "", project_id: "", date: "", start_time: "", end_time: "", event_type: "install", description: "" });
  ...
  (serviceCalls || []).forEach(sc => { const d = toDS(sc.scheduled_date); if (d) items.push({ id: sc.id, _id: "sc-" + sc.id, type: "sc", date: d, time: sc.scheduled_time, priority: sc.priority, status: sc.status, userName: sc.assigned_name, userColor: sc.assigned_color, userId: sc.assigned_to, description: sc.description, address: sc.address || sc.project_address, clientName: sc.client_name, scheduled_date: sc.scheduled_date, scheduled_time: sc.scheduled_time }); });
  ...
  function openItem(it) {
    setSel(it);
    if (it.type === "event") {
      setEditForm({ date: it.date, start_time: it.start_time || "", end_time: it.end_time || "", user_id: it.userId || "", description: it.description || "" });
    } else {
      setEditForm({ scheduled_date: it.date, scheduled_time: it.time || "", assigned_to: it.userId || "", status: it.status || "open" });
    }
  }
  ...
  async function saveEdit() {
    ...
    try {
      if (sel.type === "event") {
        await api("/schedule/" + sel.id, { method: "PATCH", body: JSON.stringify(editForm) });
      } else {
        await api("/service-calls/" + sel.id, { method: "PATCH", body: JSON.stringify(editForm) });
      }
  ...
  async function deleteEvent() {
    if (!sel || sel.type !== "event") return;
    if (!confirm("Delete this schedule event?")) return;
    try { await api("/schedule/" + sel.id, { method: "DELETE" }); setSel(null); rEv(); }
    catch (e) { alert(e.message); }
  }

  async function createEvent() {
    if (!newForm.user_id || !newForm.date) { alert("Assign a team member and date"); return; }
    setBusy(true);
    try {
      await api("/schedule", { method: "POST", body: JSON.stringify(newForm) });
      setShowNew(false); setNewForm({ user_id: "", project_id: "", date: "", start_time: "", end_time: "", event_type: "install", description: "" }); rEv();
    } catch (e) { alert(e.message); }
    setBusy(false);
  }
  ...
  return (<div>
    <div className="flex items-center justify-between mb-4 gap-4 flex-wrap">
      ...
      <div className="flex gap-2">
        ...
        <div className="flex gap-1">{["month", "list"].map(v => <button key={v} onClick={() => setView(v)} className={cn("text-xs px-3 py-1.5 rounded-lg capitalize", view === v ? "bg-brand-400/10 text-brand-400" : "text-zinc-500 hover:text-zinc-300")}>{v}</button>)}</div>
      </div>
    </div>
    ...
    {view === "list" && <div>
      ...
      : listDates.map(date => <div key={date} className="mb-6">
        ...
        <div className="space-y-2">{gr[date].map(it => <Card key={it._id} onClick={() => openItem(it)} className={cn("p-4", it.type === "sc" && "border-l-2 border-l-orange-500/50")}>
          {it.type === "sc" ? <div><div className="flex gap-2 mb-1"><Badge variant="urgent">Service Call</Badge><Badge variant={it.priority}>{it.priority}</Badge></div><p className="text-sm text-zinc-200">{it.description}</p><p className="text-xs text-zinc-500 mt-1">{it.userName || "Unassigned"} {it.time && " · " + it.time} {it.address && " · " + it.address}</p></div>
          : <div className="flex items-center gap-2.5"><Avatar name={it.userName} color={it.userColor} size="sm" /><div><div className="flex items-center gap-2"><span className="text-sm font-medium text-zinc-100">{it.userName}</span><Badge variant={it.eventType === "install" ? "in-progress" : "scheduled"}>{it.eventType}</Badge></div><p className="text-xs text-zinc-500 mt-0.5">{it.shift || it.time}</p>{it.description && <p className="text-xs text-zinc-500">{it.description}</p>}{it.project && <p className="text-xs text-zinc-600">{it.project.address}</p>}</div></div>}
        </Card>)}</div></div>)}
    </div>}
    ...
    <Modal open={!!sel} onClose={() => { setSel(null); setEditForm(null); }} title={sel?.type === "sc" ? "Edit Service Call" : "Edit Schedule Event"}>
      {sel && editForm && <div className="space-y-3">
        <div className="flex gap-2 mb-2">{sel.type === "sc" ? <><Badge variant={sel.priority}>{sel.priority}</Badge><Badge variant={sel.status}>{sel.status}</Badge></> : <Badge variant={sel.eventType === "install" ? "in-progress" : "scheduled"}>{sel.eventType}</Badge>}</div>
        ...
        {sel.type === "event" ? <>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Input label="Start Time" value={editForm.start_time || ""} onChange={e => setEditForm({...editForm, start_time: e.target.value})} placeholder="8:00 AM" />
            <Input label="End Time" value={editForm.end_time || ""} onChange={e => setEditForm({...editForm, end_time: e.target.value})} placeholder="5:00 PM" />
          </div>
          <Select label="Assigned Tech" value={editForm.user_id || ""} onChange={e => setEditForm({...editForm, user_id: e.target.value})}>
          ...
          <div className="flex gap-2">
            ...
            {isAdmin() && <Button variant="danger" onClick={deleteEvent}>Delete</Button>}
          </div>
        </> : <>
          <Input label="Scheduled Date" type="date" value={toDS(editForm.scheduled_date) || ""} onChange={e => setEditForm({...editForm, scheduled_date: e.target.value})} />
          <Input label="Scheduled Time" value={editForm.scheduled_time || ""} onChange={e => setEditForm({...editForm, scheduled_time: e.target.value})} placeholder="9:00 AM" />
          <Select label="Assigned Tech" value={editForm.assigned_to || ""} onChange={e => setEditForm({...editForm, assigned_to: e.target.value})}>
            <option value="">Unassigned</option>
            {(users || []).map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
          </Select>
          <Select label="Status" value={editForm.status || "open"} onChange={e => setEditForm({...editForm, status: e.target.value})}>
    ...
    <Modal open={showNew} onClose={() => setShowNew(false)} title="Schedule Visit">
      <div className="space-y-3">
        <Select label="Assign Tech *" value={newForm.user_id} onChange={e => setNewForm({...newForm, user_id: e.target.value})}>
          <option value="">Select team member...</option>
          {(users || []).map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
        </Select>
        <Select label="Project" value={newForm.project_id} onChange={e => setNewForm({...newForm, project_id: e.target.value})}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Input label="Start Time" value={newForm.start_time} onChange={e => setNewForm({...newForm, start_time: e.target.value})} placeholder="8:00 AM" />
          <Input label="End Time" value={newForm.end_time} onChange={e => setNewForm({...newForm, end_time: e.target.value})} placeholder="5:00 PM" />
        </div>
        <Select label="Type" value={newForm.event_type} onChange={e => setNewForm({...newForm, event_type: e.target.value})}>
          <option value="install">Install</option><option value="meeting">Meeting</option><option value="task">Task</option>
        </Select>
