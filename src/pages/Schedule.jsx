...
export default function Schedule() {
  ...
  function openItem(it) {
    setSel(it);
    if (it.type === "event") {
      setEditForm({ date: it.date, start_time: it.start_time || "", end_time: it.end_time || "", user_id: it.userId || "", attendee_ids: (it.attendees || []).map(a => a.id), description: it.description || "" });
    } else {
  ...
  async function createEvent() {
    ...
    try {
      ...
      setShowNew(false); setNewForm({ user_id: "", attendee_ids: [], project_id: "", date: "", start_time: "", end_time: "", event_type: "install", description: "" }); rEv();
    } catch (e) { alert(e.message); } setBusy(false);
  }
    ...
    {view === "list" && <div>
      ...
      : listDates.map(date => <div key={date} className="mb-6">
        ...
        <div className="space-y-2">{gr[date].map(it => <Card key={it._id} onClick={() => openItem(it)} className={cn("p-4", it.type === "sc" && "border-l-2 border-l-orange-500/50")}>
          ...
          : <div className="flex items-center gap-2.5"><Avatar name={it.userName} color={it.userColor} size="sm" /><div><div className="flex items-center gap-2"><span className="text-sm font-medium text-zinc-100">{it.userName}</span>{it.attendees?.map(a => <Avatar key={a.id} name={a.name} color={a.color} size="sm" />)}<Badge variant={it.eventType === "install" ? "in-progress" : "scheduled"}>{it.eventType}</Badge></div><p className="text-xs text-zinc-500 mt-0.5">{it.shift || it.time}</p>{it.description && <p className="text-xs text-zinc-500">{it.description}</p>}{it.project && <p className="text-xs text-zinc-600">{it.project.address}</p>}</div></div>}
        </Card>)}</div></div>)}
    </div>}
