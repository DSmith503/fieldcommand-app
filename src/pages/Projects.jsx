...
export default function ProjectDetail() {
  ...
  const [newAreaTasks, setNewAreaTasks] = useState(['']);
  ...
  async function addTaskToArea(areaId) {
    if (!newTaskText.trim()) return;
    try {
      await api('/projects/' + id + '/areas/' + areaId + '/tasks', { method: 'POST', body: JSON.stringify({ text: newTaskText }) });
      setNewTaskText(''); setShowAddTask(null); load();
    } catch (e) { alert(e.message); }
  }
  async function createArea() {
    if (!newAreaName.trim()) return;
    const tasks = newAreaTasks.filter(t => t.trim());
    try {
      await api('/projects/' + id + '/areas', { method: 'POST', body: JSON.stringify({ name: newAreaName, tasks }) });
      setNewAreaName(''); setNewAreaTasks(['']); setShowAddArea(false); load();
    } catch (e) { alert(e.message); }
  }
  ...
  return (
    <div>
            ...
            {(project.areas || []).map(area => <div key={area.id} className="mb-6"><h3 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2 px-1">{area.name}</h3><div className="space-y-1">{(area.tasks || []).map(task => (
              <div key={task.id} className={cn('rounded-xl text-sm bg-[#0a0a0c] border border-white/[0.04]', task.done && 'opacity-70')}>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <input value={newTaskText} onChange={e => setNewTaskText(e.target.value)} onKeyDown={e => e.key === 'Enter' && addTaskToArea(area.id)} placeholder="New task..." className="flex-1 bg-white/[0.025] border border-brand-400/50 rounded-lg px-3 py-2 text-sm text-zinc-100 outline-none" autoFocus />
                  <Button onClick={() => addTaskToArea(area.id)} className="text-xs py-1.5 px-3">Add</Button>
                  <button onClick={() => { setShowAddTask(null); setNewTaskText(''); }} className="text-xs text-zinc-500 hover:text-zinc-300">Cancel</button>
                </div>
              ) : (
                <button onClick={() => setShowAddTask(area.id)} className="text-[11px] text-zinc-500 hover:text-brand-400 flex items-center gap-1 mt-1.5 px-1"><Plus className="w-3 h-3" /> Add Task</button>
              )}</div></div>)}
      ...
      <Modal open={showAddArea} onClose={() => { setShowAddArea(false); setNewAreaName(''); setNewAreaTasks(['']); }} title="Add Area"><div className="space-y-3">
        <Input label="Area Name *" placeholder="e.g. Outdoor Patio" value={newAreaName} onChange={e => setNewAreaName(e.target.value)} />
        <div><label className="text-xs font-medium text-zinc-400 mb-1.5 block">Tasks</label>
          {newAreaTasks.map((t, i) => <div key={i} className="flex gap-2 mb-1.5"><input value={t} onChange={e => { const copy = [...newAreaTasks]; copy[i] = e.target.value; setNewAreaTasks(copy); }} placeholder={"Task " + (i + 1)} className="flex-1 bg-white/[0.025] border border-white/[0.06] rounded-xl px-3.5 py-2 text-sm text-zinc-100 outline-none focus:border-brand-400/50 placeholder-zinc-600" />{newAreaTasks.length > 1 && <button onClick={() => setNewAreaTasks(newAreaTasks.filter((_, j) => j !== i))} className="text-zinc-600 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>}</div>)}
          <button onClick={() => setNewAreaTasks([...newAreaTasks, ''])} className="text-[11px] text-zinc-500 hover:text-brand-400 flex items-center gap-1"><Plus className="w-3 h-3" /> Add another task</button>
        </div>
