...
export default function Team() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', role: 'technician', hourly_rate: '', color: '#60A5FA' });
  const [busy, setBusy] = useState(false);

  async function submit() {
    if (!form.name || !form.email || !form.password) { alert('Name, email, and password required'); return; }
    setBusy(true);
    try { await api('/users', { method: 'POST', body: JSON.stringify({ ...form, hourly_rate: Number(form.hourly_rate) || 0 }) }); setShowNew(false); setForm({ name: '', email: '', password: '', role: 'technician', hourly_rate: '', color: '#60A5FA' }); reload(); }
      ...
      {(users || []).map(u => (
        <Card key={u.id} className="p-4 mb-2 flex items-center gap-4">
          ...
          <div className="text-right flex-shrink-0">
            <Badge variant={u.role === 'admin' ? 'admin' : 'employee'}>{u.role}</Badge>
            {u.role !== 'admin' && u.hourly_rate > 0 && <p className="text-xs text-zinc-500 mt-1">{fmtMoney(u.hourly_rate)}/hr</p>}
          </div>
        </Card>
      ))}
